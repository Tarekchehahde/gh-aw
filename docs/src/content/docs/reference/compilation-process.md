...
| **firewall-logs/** | `/tmp/gh-aw/sandbox/firewall/logs/` on GitHub-hosted runners; `${{ runner.temp }}/gh-aw/sandbox/firewall/logs/` when `runner.topology: arc-dind` | Network access logs in Squid format (when `network.firewall:` enabled) | Analyzed by `gh aw logs` command |
