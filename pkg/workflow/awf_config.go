...
func BuildAWFConfigJSON(config AWFCommandConfig) (string, error) {
	...
	if awfImageTag != "" || isArcDindTopology(config.WorkflowData) {
		...
		// For ARC/DinD topology, set dockerHostPathPrefix in config instead of CLI flag.
		...
		if isArcDindTopology(config.WorkflowData) {
			container.DockerHostPathPrefix = "/tmp/gh-aw"
			awfConfigLog.Printf("Container section: dockerHostPathPrefix=/tmp/gh-aw (arc-dind topology)")
		}
