...
CLI-proxy tools (call via `bash` only, not as MCP): `playwright-cli`, `serena`, `agenticworkflows`, `safeoutputs`, `mcpscripts`. GitHub access is via `gh` CLI (`tools.github.mode: gh-proxy`), not the `github` MCP tool.
