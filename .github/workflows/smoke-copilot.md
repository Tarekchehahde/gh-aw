...
5. **`web-fetch` tool**: fetch `https://github.com` and confirm response contains `GitHub`. If `web_fetch` is unavailable, use `curl -sL https://github.com` via bash and mark ❌ only if both fail.
